...
class CreateExtraDenunciadoFisicoTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('extra_denunciado_fisico', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('idVariablesPersona')->unsigned();
            //$table->integer('idNotificacion')->unsigned();
            $table->integer('idPuesto')->unsigned()->nullable()->default(9);
    ...
    {
        Schema::dropIfExists('extra_denunciado_fisico');
    }
}
