    function getRFCpf(nombreInput,primerApInput,segundoApInput,fechaNacInput){
        var inNombre=$("input[name='"+nombreInput+"']");
        var inPrimerAp=$("input[name='"+primerApInput+"']");
        var inSegundoAp=$("input[name='"+segundoApInput+"']");
         var inFechaNacimiento=$("input[name='"+fechaNacInput+"']");//yyyy
      
        var ap_paterno = inPrimerAp.val();
        var ap_materno = inSegundoAp.val();
        var nombre = inNombre.val();
        var dteNacimiento =  inFechaNacimiento.val();
        var rfc;
        
        if(nombre && ap_paterno && ap_materno && dteNacimiento){

            if(isNaN(dteNacimiento[2]))
                dteNacimiento =  dteNacimiento.substr(8,2)+dteNacimiento.substr(3,2)+dteNacimiento.substr(0,2);
            else
                dteNacimiento =  dteNacimiento.substr(2,2)+dteNacimiento.substr(5,2)+dteNacimiento.substr(8,2);
...
function getCURP(nombreInput,primerApInput,segundoApInput,sexoInput,fechaNacInput,estadoInput){
    var inNombre=$("input[name='"+nombreInput+"']");
    var inPrimerAp=$("input[name='"+primerApInput+"']");
    var inSegundoAp=$("input[name='"+segundoApInput+"']");
    var inSexo=$("input[name='"+sexoInput+"']");
    var inFechaNacimiento=$("input[name='"+fechaNacInput+"']");//yyyy-mm-dd
    var inEdoOrigen=$("#"+estadoInput);
    ...
    var mes=null;
    var anio=null;
    var renapo = [ 'AS', 'BC', 'BS', 'CC', 'CS', 'CH', 'CL', 'CM', 'DF', 'DG',
        'GT', 'GR', 'HG', 'JC', 'MC', 'MN', 'MS', 'NT', 'NL', 'OC', 'PL', 'QT',
        'QR', 'SP', 'SL', 'SR', 'TC', 'TS', 'TL', 'VZ', 'YN', 'ZS', 'NE' ];

    var nombreVal,primerVal,segundoVal,sexoVal,fechaVal,edoVal;
    
    nombreVal=inNombre.val();
    primerVal=inPrimerAp.val();
    segundoVal=inSegundoAp.val();

    if(inSexo.attr('type')==='radio'){
        $.each( inSexo, function( key, item ) {
          if(item.checked){
            switch($('#'+item.id).val()){
                case 'hombre': 
                case 'h':
    ...
    }else if(inSexo.attr('type')==='undefined'){
        inSexo=$("#"+sexoInput);
        if (inSexo && inSexo.select2){
            switch(inSexo.val()){
                case 'hombre': 
                case 'h':
                    sexoVal='H';
    ...
    fechaVal=inFechaNacimiento.val();
    
    edoVal=(inEdoOrigen.val())?inEdoOrigen.val():null;

    if(nombreVal && primerVal && segundoVal && fechaVal && sexoVal && edoVal){

        if(isNaN(fechaVal[2])){
            anio=fechaVal.substr(8, 2);
            mes=fechaVal.substr(3, 2);
            dia=fechaVal.substr(0, 2);
        }else{
            anio=fechaVal.substr(2, 2);
            mes=fechaVal.substr(5, 2);
            dia=fechaVal.substr(8, 2);
        }

        edoVal=renapo[edoVal-1];

        var gen=generaCurp({
              nombre            : nombreVal,
              apellido_paterno  : primerVal,
              apellido_materno  : segundoVal,
              sexo              : sexoVal,
              estado            : edoVal,
...
function getRFCpm(razonInput,fechaConstInput){//
        var inRazon=$("input[name='"+razonInput+"']");
        var inFechaConst=$("input[name='"+fechaConstInput+"']");//yyyy-mm-dd

        var razon = inRazon.val();
        var dteFechaConst=inFechaConst.val();
        var rfc;
        
        if(razon && dteFechaConst){
            if(isNaN(dteFechaConst[2]))
                dteFechaConst =  dteFechaConst.substr(8,2)+dteFechaConst.substr(3,2)+dteFechaConst.substr(0,2);
            else
                dteFechaConst =  dteFechaConst.substr(2,2)+dteFechaConst.substr(5,2)+dteFechaConst.substr(8,2);
            //FILTRA ACENTOS
            var razon_f = RFCFiltraAcentos(razon.toLowerCase());
            //GUARDA NOMBRE ORIGINAL PARA GENERAR HOMOCLAVE
            var razon_orig = razon_f;
            //ELIMINA PALABRAS SOBRANTES DE LOS NOMBRES
            razon_f = RFCpmFiltraNombres(razon_f);

           rfc = RFCpmArmalo(razon_f)+dteFechaConst;
...
function RFCpmArmalo(razon){

        var rfc;
        var palabras=razon.split(' ');
        var palabrasNum=[];
        var aux=[],aux2=[];
        var i;
        ...
        for(i=0;i<palabras.length;i++){
            if((palabras[i]!='undefined')&&(palabras[i].trim()!='')&&(palabras[i]!=null))
                palabrasNum.push(number2string(isRomanNumberConvert(palabras[i].trim())));
        }

        for(i=0;i<palabrasNum.length;i++){
            aux=palabrasNum[i].split(' ');
        ...
        switch(palabrasNum.length){
            case 1:
                switch(palabrasNum[0].length){
                    case 1:
                        rfc=palabrasNum[0][0]+'xx';
                        break;
                    case 2:
                        rfc=palabrasNum[0][0]+palabrasNum[0][1]+'x';
                        break;
                    default:
                        rfc=palabrasNum[0][0]+palabrasNum[0][1]+palabrasNum[0][2];
                }                
                break;
            case 2:
                rfc=palabrasNum[0][0];
                switch(palabrasNum[1].length){
                    case 1:
                        rfc=rfc+palabrasNum[1][0]+'x';
                        break;
                    default:
                        rfc=rfc+palabrasNum[1][0]+palabrasNum[1][1];
                }                
                break;
            default:
                rfc=palabrasNum[0][0]+palabrasNum[1][0]+palabrasNum[2][0];
                break;
        }
...
function RFCpmFiltraNombres(strTexto){
        var i = 0;


        strTexto = strTexto.replace(',','');

        var strArPalabras = ["s. en n.c.","s. en c.", "s. de r.l.", "s. en c. por a.", "s.a. de c.v.", "a. en p.", "s.n.c.", "s.c.", "s.c.l.", "s.c.s.","s.a.","a.c.","c.v.","r.l."," s."," a."," c."," v."," l."," n."," p."," p."];
        for (i=0;i<=strArPalabras.length;i++){
            strTexto = strTexto.replace(strArPalabras[i],"");
        }


        strArPalabras = [" de "," del ", "del ", "la ", "los ", "las ", "y ", "mc ", "mac ", "von ", "van ", "el ", "la ", "los ", "las ", " a ", "para "," de "];
        for (i=0;i<=strArPalabras.length;i++){
            strTexto = strTexto.replace(strArPalabras[i]," ");
        }


        strArPalabras = ["compañia","cia. ","sociedad","soc. ",".",","];
        for (i=0;i<=strArPalabras.length;i++){
            strTexto = strTexto.replace(strArPalabras[i]," ");
        }

        strArPalabras = ["@ ", "% ","( ", "# ", "/ "];
        strNames = ["arroba ", "porcentaje ","abre parentesis ", "numeral ", "diagonal "];
        for (i=0;i<=strArPalabras.length;i++){
            strTexto = strTexto.replace(strArPalabras[i],strNames[i]);
        }
...
function isRomanNumberConvert(cadeval){


    var orig=cadeval;
    var letrasRomanas = ['i','v','x','l','c','d','m'];
    var i;
    var isRoman=true;
    cadeval=cadeval.replace(' ','');
    for(i=0;i<cadeval.length;i++){
        if(letrasRomanas.indexOf(cadeval[i])>=0)
            continue;
        else{
            isRoman=false;
            break;
    ...
    if(isRoman){
        cadeval=fromRoman(cadeval);
        return cadeval==0?'':cadeval;
    }else
        return orig;
}
...
function number2string(numero){
    if(isNaN(numero))
        return numero;
    else
        return aLetra(numero);
}
